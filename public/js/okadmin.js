...
//打开tab页
function addTabNew(title,url,id) {
  //判断是否存在tab，可以通过查看源码确定获取方式是否是li[lay-id='id']
  var exist = $("li[lay-id='" + id + "']").length; 
  //如果不存在则新增一个tab
  if (exist == 0) {
      var openTitle = '<strong style="display: none;" is-close="true" lay-id='+id+'></strong><cite>' + title + '</cite>';
      layui.element.tabAdd("ok-tab", {
          title: openTitle,
          content: "<iframe src='" + url + "' lay-id='" + id + "' frameborder='0' scrolling='yes' width='100%' height='100%'></iframe>",
          id: id
      });
  }
  //切换到当前tab页
  layui.element.tabChange("ok-tab", id);
}
window.test= function(title, url, id){
  addTabNew(title, url, id);
};
  /**
   * 左边菜单显隐功能
   */
